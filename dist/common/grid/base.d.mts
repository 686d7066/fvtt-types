...
export default class BaseGrid<Coordinates2D = GridCoordinates2D, Coordinates3D = GridCoordinates3D> {
     ...
     * @param {Point} origin        The origin point of the rectangle.
     * @param {number} width        The width in grid units.
     * @param {number} height       The height in grid units.
     * @param {Point} anchor        The anchor.
     * @param {number} rotation     The rotation in degrees.
     * @returns {Point[]}           The points of the rectangle polygon.
     */
    getRectangle(origin: Point, width: number, height: number, anchor: Point, rotation: number): Point[];
    /**
     * Get the line polygon given the length and width in grid units for this grid.
     * The points of the polygon are returned ordered in positive orientation.
     * @param {Point} origin        The origin point of the line.
     * @param {number} length       The length in grid units.
     * @param {number} width        The width in grid units.
     * @param {number} direction    The direction in degrees.
     * @returns {Point[]}           The points of the line polygon.
     */
    getLine(origin: Point, length: number, width: number, direction: number): Point[];
    /**
     ...
     * @param {number} radiusX    The x-radius in grid units.
     * @param {number} radiusY    The y-radius in grid units.
     * @param {number} rotation   The rotation in degrees.
     * @returns {Point[]}         The points of the ellipse polygon.
     * @abstract
     */
    getEllipse(center: Point, radiusX: number, radiusY: number, rotation: number): Point[];
    /**
     ...
     * @param {Point} center                                      The center point of the ring.
     * @param {number} radius                                     The radius in grid units.
     * @param {number} innerWidth                                 The inner width in grid units.
     * @param {number} outerWidth                                 The outer width in grid units.
     * @returns {[innerCircle: Point[], outerCircle: Point[]]}    The inner and outer circles of the ring polygon.
     */
    getRing(center: Point, radius: number, innerWidth: number, outerWidth: number): [innerCircle: Point[], outerCircle: Point[]];
}
