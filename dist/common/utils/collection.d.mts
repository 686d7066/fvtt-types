...
export default class Collection<K extends string, V> extends Map<K, V> {
     ...
     * @see Array#forEach
     * @param {(value: V, index: number) => void} fn A function to apply to each element
     *
     ...
     * ```
     */
    forEach(fn: (value: V, index: number) => void): void;
    /**
     ...
     * @see {Array#map}
     * @template U
     * @param {(element: V, index: number, collection: Collection<K, V>) => U} transformer A transformation function
     *                                                                                     applied to each entry value.
     * @returns {U[]} An Array of transformed values
     */
    map<U>(transformer: (element: V, index: number, collection: Collection<K, V>) => U): U[];
    /**
     * Reduce the Collection by applying an evaluator function and accumulating entries
     * @see {Array#reduce}
     * @template U
     * @param {(accum: U, element: T, index: number, collection: Collection<K, V>) => U} reducer A reducer function
     *                                                                                           applied to each entry
     *                                                                                           value.
     * @param {U} initial An initial value which accumulates with each iteration
     ...
     * ```
     */
    reduce<U>(reducer: (accum: U, element: T, index: number, collection: Collection<K, V>) => U, initial: U): U;
    /**
     * Test whether a condition is met by some entry in the Collection.
     * @see {Array#some}
     * @param {(element: V, index: number, set: Collection<K, V>) => boolean} condition The functional condition to test.
     * @returns {boolean}  Was the test condition passed by at least one entry?
     */
    some(condition: (element: V, index: number, set: Collection<K, V>) => boolean): boolean;
    /**
     * Test whether a condition is met by every entry in the Collection.
     * @see {Array#every}
     * @param {(element: V, index: number, set: Collection<K, V>) => boolean} condition  The functional condition to test.
     * @returns {boolean}  True if the test condition was truthy for every entry.
     */
    every(condition: (element: V, index: number, set: Collection<K, V>) => boolean): boolean;
    /**
